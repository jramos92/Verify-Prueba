...
public class zzkv
{
  ...
  public static String zza(Calendar paramCalendar)
  {
    ...
    return ((String)localObject).replace("+0000", zzZK[0]);
  }
  ...
  public static Calendar zzbT(String paramString)
  {
    ...
    Object localObject2 = zzbU(paramString);
    if (TextUtils.isEmpty((CharSequence)localObject2))
    {
      zzVo.zzb("Invalid date format", new Object[0]);
      return null;
    }
    String str = zzbV(paramString);
    ...
    for (paramString = "yyyyMMdd'T'HHmmss";; paramString = zzZL)
    {
      ...
      {
        label127:
        zzVo.zzb("Error parsing string: %s", new Object[] { paramString.getMessage() });
      }
    }
    return null;
  }
  ...
  private static String zzbU(String paramString)
  {
    ...
    {
      zzVo.zze("Error extracting the date: %s", new Object[] { paramString.getMessage() });
    }
    return null;
  }
  
  private static String zzbV(String paramString)
  {
    if (TextUtils.isEmpty(paramString)) {
    ...
    do
    {
      do
      {
        ...
        {
          zzVo.zzb("Error extracting the time substring: %s", new Object[] { paramString.getMessage() });
          return null;
        }
      ...
      } while (!zzbW(paramString));
      return paramString.replaceAll("([\\+\\-]\\d\\d):(\\d\\d)", "$1$2");
    } while (paramString.length() != "HHmmss".length() + zzZK[0].length());
    return paramString.substring(0, paramString.length() - 1) + "+0000";
  }
  
  private static boolean zzbW(String paramString)
  {
...
/* Location:              C:\Users\julian\Downloads\Veryfit 2 0_vV2.0.28_apkpure.com-dex2jar.jar!\com\google\android\gms\internal\zzkv.class
